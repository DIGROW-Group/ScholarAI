...
const FileUpload = ({ 
  onFilesSelected, 
  maxFiles = 3,
  // Optional maxSize provided in BYTES (e.g., 16 * 1024 * 1024 for 16MB). Leave undefined for no limit.
  maxSize,
  acceptedTypes = ['.pdf']
}) => {
  ...
  const onDrop = useCallback(async (acceptedFiles, rejectedFiles) => {
    ...
    // Handle accepted files with optional size validation
    if (acceptedFiles.length > 0) {
      const sizeCheckResults = acceptedFiles.map((file) => {
        try {
          // Size validation when a maxSize is provided
          if (typeof maxSize === 'number' && maxSize > 0 && file.size > maxSize) {
            const maxSizeLabel = formatFileSize(maxSize);
            toast.error(
              maxSizeLabel 
                ? `File ${file.name} is too large. Maximum size is ${maxSizeLabel}.`
