...
const ProfilesList = () => {
  ...
  // KPI Review modal state
  const [showKPIModal, setShowKPIModal] = useState(false);
  const [kpiProfileId, setKpiProfileId] = useState(null);
  
  // Auto-refresh functionality
  const refreshIntervalRef = useRef(null);
  const [isAutoRefreshEnabled, setIsAutoRefreshEnabled] = useState(false);
  
  // Helper functions to manage notified profiles in sessionStorage
  const getNotifiedProfileIds = useCallback(() => {
    try {
      const stored = sessionStorage.getItem('kpi_notified_profiles');
      return stored ? new Set(JSON.parse(stored)) : new Set();
    } catch (error) {
      console.error('Error reading notified profiles from sessionStorage:', error);
      return new Set();
    }
  }, []);

  const addNotifiedProfileId = useCallback((profileId) => {
    try {
      const notifiedIds = getNotifiedProfileIds();
      notifiedIds.add(profileId);
      sessionStorage.setItem('kpi_notified_profiles', JSON.stringify(Array.from(notifiedIds)));
    } catch (error) {
      console.error('Error saving notified profile to sessionStorage:', error);
    }
  }, [getNotifiedProfileIds]);

  const removeNotifiedProfileId = useCallback((profileId) => {
    try {
      const notifiedIds = getNotifiedProfileIds();
      notifiedIds.delete(profileId);
      sessionStorage.setItem('kpi_notified_profiles', JSON.stringify(Array.from(notifiedIds)));
    } catch (error) {
      console.error('Error removing notified profile from sessionStorage:', error);
    }
  }, [getNotifiedProfileIds]);

  const isProfileNotified = useCallback((profileId) => {
    return getNotifiedProfileIds().has(profileId);
  }, [getNotifiedProfileIds]);
  ...
  useEffect(() => {
    if (location.state?.refreshNeeded) {
      ...
      // Clear the navigation state immediately to prevent re-showing the message
      window.history.replaceState({}, document.title);
      
      // Show toast after clearing state to prevent duplicate notifications
  ...
  // Show notification for profiles ready for KPI review (only once per profile, persists across page navigations)
  useEffect(() => {
    if (!loading && profiles.length > 0) {
      const profilesNeedingReview = profiles.filter(hasKPIsReadyForReview);
      
      // Find profiles that need review but haven't been notified yet
      const newProfilesNeedingReview = profilesNeedingReview.filter(profile => {
        return !isProfileNotified(profile.id);
      });
      
      // Show notification for new profiles ready for review
      if (newProfilesNeedingReview.length > 0) {
        newProfilesNeedingReview.forEach(profile => {
          // Mark this profile as notified in sessionStorage
          addNotifiedProfileId(profile.id);
          // Show notification only once
          toast.success(`KPIs extracted for ${profile.company_name}! Click the edit icon to review.`, { duration: 5000 });
        });
      }
      
      // Clean up: Remove profiles that are no longer in "kpis_extracted" state from sessionStorage
      // This allows re-notification if a profile is reprocessed
      const notifiedIds = getNotifiedProfileIds();
      const currentProfileIds = new Set(profiles.map(p => p.id));
      
      notifiedIds.forEach(profileId => {
        if (!currentProfileIds.has(profileId)) {
          // Profile no longer exists, remove from sessionStorage
          removeNotifiedProfileId(profileId);
        } else {
          // Profile exists, check if it's still in kpis_extracted state
          const profile = profiles.find(p => p.id === profileId);
          if (profile && !hasKPIsReadyForReview(profile)) {
            // Profile is no longer in kpis_extracted state, remove from sessionStorage
            removeNotifiedProfileId(profileId);
          }
        }
      });
    }
  }, [profiles, loading, isProfileNotified, addNotifiedProfileId, removeNotifiedProfileId, getNotifiedProfileIds]);
  ...
  const handleDelete = async () => {
    ...
    await executeWithLoading(
      ...
      {
        setLoading: (loading) => setDeleteLoading(loading ? profileToDelete.id : null),
        errorMessage: 'Failed to delete profile'
      }
    );
  };

  const getStatusBadge = (status, profile) => {
    // Check processing stage for more specific status
    const processingStage = profile?.profile_data?.processing_stage;
    
    let displayStatus = status;
    let statusColor = 'bg-gray-100 text-gray-800';
    
    if (status === 'processing') {
      if (processingStage === 'kpis_extracted') {
        displayStatus = 'KPIs Ready for Review';
        statusColor = 'bg-blue-100 text-blue-800';
      } else if (processingStage === 'generating_report') {
        displayStatus = 'Generating Report';
        statusColor = 'bg-yellow-100 text-yellow-800';
      } else {
        displayStatus = 'Extracting KPIs';
        statusColor = 'bg-yellow-100 text-yellow-800';
      }
  ...
  // Check if profile has KPIs ready for review (not completed yet)
  const hasKPIsReadyForReview = (profile) => {
    const processingStage = profile?.profile_data?.processing_stage;
    const isCompleted = profile.status === 'completed';
    // Only show edit button if KPIs are extracted but profile is not yet completed
    return processingStage === 'kpis_extracted' && !isCompleted;
  };

  // Navigate to KPI review page
  const reviewKPIs = (profile) => {
  ...
  const reprocessProfile = async (profileId) => {
    await executeWithLoading(
      ...
      {
        setLoading: (loading) => setReprocessLoading(loading ? profileId : null),
  ...
  const sendProfileEmail = async (profile) => {
    ...
    await executeWithLoading(
      ...
      {
        setLoading: (loading) => setEmailLoading(loading ? profile.id : null),
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {profiles.map((profile, index) => (
                    ...
                    >
                      ...
                      <td className="table-cell">
                        {getStatusBadge(profile.status, profile)}
                      </td>
                      <td className="table-cell">
                      ...
                      <td className="table-cell">
                        <div className="flex items-center space-x-2">
                          {hasKPIsReadyForReview(profile) && (
                            <button 
                              onClick={() => reviewKPIs(profile)} 
                              className="p-1 text-gray-400 hover:text-blue-600 transition-colors"
                              title="Review and confirm KPIs"
                            >
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <div>{getStatusBadge(selectedProfile.status, selectedProfile)}</div>
                  </div>
                </div>
