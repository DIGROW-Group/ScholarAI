...
function computeAndDisplayCAGR(kpiName, cellId) {
    ...
    if (current != null && previous != null && !isNaN(current) && !isNaN(previous)) {
        // Check if we can calculate CAGR normally (when previous != 0 and both values are positive)
        if (previous !== 0 && current >= 0 && previous >= 0) {
            ...
            // When previous value is 0 or when dealing with negative values, show absolute difference
            const difference = current - previous;
            document.getElementById(cellId).innerHTML = renderDifferenceCell(difference);
        }
    } else {
...
function renderDifferenceCell(difference) {
    if (difference == null || isNaN(difference)) {
        return `<span class="change" style="font-size: 1.5em; text-align: center; width: 100%;">-</span>`;
    }

    const isPositive = difference >= 0;
    const formatted = formatFinancialNumber(difference, 'MAD');

    return `
    <span class="change ${isPositive ? 'positive' : 'negative'}">
        ${isPositive ? '+' : ''}${formatted} <i class="fas fa-arrow-${isPositive ? 'up' : 'down'}"></i>
    </span>`;
}    
...
function generateFinancialInsights() {
    ...
    try {
        ...
        let cagrText = "";
        if (revenueN != null && revenueN1 != null && !isNaN(revenueN) && !isNaN(revenueN1)) {
            // Check if we can calculate percentage growth (when previous != 0 and both values are positive)
            if (revenueN1 !== 0 && revenueN >= 0 && revenueN1 >= 0) {
                // Calculate percentage growth when previous value is not zero and both values are positive
                ...
                // When previous value is zero or when dealing with negative values, show absolute difference
                const difference = revenueN - revenueN1;
                const formattedDiff = formatFinancialNumber(Math.abs(difference), 'MAD');
                if (difference > 0) {
                    cagrText = `croissance de ${formattedDiff}`;
                } else if (difference < 0) {
                    cagrText = `contraction de ${formattedDiff}`;
                } else {
                    cagrText = "stabilité";
                }
            }
...
function downloadExcelReport() {
    try {
        ...
        const profileIdMatch = currentPath.match(/\/profiles\/([^\/]+)\/report/);
        
        if (!profileIdMatch) {
            console.error('Could not extract profile ID from URL');
            showToast('Erreur: Impossible de déterminer l\'ID du profil', 'error');
            return;
        }
        
        const profileId = profileIdMatch[1];
        console.log('Downloading Excel report for profile:', profileId);
        
        // Show loading state
        const downloadBtn = document.getElementById('download-excel-btn');
        const originalText = downloadBtn.innerHTML;
        downloadBtn.innerHTML = '<i class="fas fa-spinner fa-spin" style="font-size: 16px;"></i> Génération...';
        downloadBtn.disabled = true;
        
        // Create download link
        const downloadUrl = `/api/profiles/${profileId}/download-excel`;
        
        // Create a temporary link element and trigger download
        const link = document.createElement('a');
        link.href = downloadUrl;
        link.download = `${companyName}_donnees_financieres.xlsx`;
        ...
        showToast('Erreur lors du téléchargement du fichier Excel', 'error');
        
        // Reset button state
        const downloadBtn = document.getElementById('download-excel-btn');
        if (downloadBtn) {
            downloadBtn.innerHTML = '<i class="fas fa-file-excel" style="font-size: 16px;"></i> Télécharger Excel';
            downloadBtn.disabled = false;
        }
    }
}
