...
def _extract_text_from_pdf(file_path: str) -> str:
    """
    Extract text content from a PDF file for debugging purposes.
...
def extract_company_info_from_first_page(file_path: str, api_key: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Extract text from the original PDF for debugging
        # print(f"[VERIFICATION] Extracting text from original PDF for debugging...", flush=True)
        # original_text = _extract_text_from_pdf(file_path)
        # print(f"[VERIFICATION] Original PDF text (first 1000 chars):", flush=True)
        # print(f"{original_text[:1000]}...", flush=True)
        
        # Initialize Anthropic client
        try:
            client = anthropic.Anthropic(api_key=api_key)
            print(f"[VERIFICATION] Anthropic client initialized successfully", flush=True)
        except Exception as client_error:
            print(f"[VERIFICATION] Error initializing Anthropic client: {client_error}", flush=True)
            return {
                'success': False,
                'error': 'Failed to initialize AI service. Please check configuration.',
        ...
        # Extract text from first page for debugging
        print(f"[VERIFICATION] Extracting text from first page PDF for debugging...", flush=True)
        first_page_text = _extract_text_from_pdf(temp_first_page_path)
        # print(f"[VERIFICATION] First page PDF text:", flush=True)
        # print(f"{first_page_text}", flush=True)
        
        # Read the first page PDF as base64 (with size limit)
        ...
        if len(pdf_data) > 10 * 1024 * 1024:  # 10MB
            log_error(f"PDF file too large for verification: {len(pdf_data)} bytes")
            return {
                'success': False,
                'error': 'Document too large for verification. Please use smaller documents.',
        ...
        extraction_prompt = (
            ...
            "- Cherche spécifiquement des mentions comme 'Société :', 'Entreprise :', 'Sté :', etc.\n"
            "- Ignore les titres de documents comme 'BILAN', 'ACTIF', 'PASSIF', etc.\n\n"
            "CRITIQUE - Distinction entre l'entreprise et le bureau comptable:\n"
            "- Le nom de l'entreprise est généralement situé en haut du document, dans le titre ou les premières lignes\n"
            "- Le nom du bureau comptable (expert-comptable) apparaît généralement en bas du document, dans un cachet, un tampon ou une section de contact avec des informations comme 'RC:', 'TP:', 'IF:', 'ICE:'\n"
            "- NE PAS utiliser le nom du bureau comptable comme nom d'entreprise, même s'il contient 'SARL', 'SA', etc.\n"
            "- Le nom de l'entreprise est celui qui apparaît dans le titre du document (ex: 'Tableau n° 1 [NOM ENTREPRISE]', 'BILAN [NOM ENTREPRISE]')\n"
            "- Si tu vois plusieurs noms d'entreprises, choisis celui qui est le sujet principal du document financier (généralement en haut)\n"
            "- Ignore complètement les noms dans les cachets, tampons, ou sections de contact en bas du document\n\n"
        ...
        print(f"[VERIFICATION] Prompt being sent to Claude:", flush=True)
        print(f"{extraction_prompt}", flush=True)
        
        # Send request to Claude with PDF document format (with timeout)
        try:
            print(f"[VERIFICATION] Sending request to Claude API...", flush=True)
            ...
            print(f"[VERIFICATION] Claude API response received successfully", flush=True)
        except Exception as api_error:
            error_message = str(api_error)
            print(f"[VERIFICATION] Claude API error: {error_message}", flush=True)
        ...
        print(f"[VERIFICATION] Claude raw response:", flush=True)
        print(f"{response_text}", flush=True)
        
        # Check if response is empty
        if not response_text:
            print(f"[VERIFICATION] Empty response from Claude API", flush=True)
            return {
                'success': False,
        ...
        print(f"[VERIFICATION] Extracted JSON text:", flush=True)
        print(f"{json_text}", flush=True)
        
        # Check if JSON extraction failed
        if not json_text:
            print(f"[VERIFICATION] Failed to extract JSON from response", flush=True)
            return {
                'success': False,
        ...
        try:
            ...
            print(f"[VERIFICATION] Parsed JSON:", flush=True)
            print(f"{company_info}", flush=True)
            ...
            if company_name and isinstance(company_name, str):
                ...
                # Return placeholder when company name is not found (poor PDF quality)
    ...
    try:
        ...
        if not all_company_info:
            ...
            if fallback_company_name:
                ...
                # Try to use a generic company name if extraction completely fails
                print(f"[VERIFICATION] No company info extracted from any document, using generic name", flush=True)
                all_company_info = [{'company_name': 'Company Name Not Extracted', 'fiscal_year': None}]
                company_names.add('Company Name Not Extracted')
        ...
        # Skip existing profile check if company name is placeholder (will be replaced by extracted name)
        existing_profile = None
        if primary_company_name and primary_company_name != "company name placeholder":
        ...
        # Skip document analysis if company name is placeholder (will be replaced by extracted name)
        document_analysis = {
            'new_documents': [{'index': i, 'file_path': path, 'reason': 'new_document'} for i, path in enumerate(file_paths)],
            'existing_matches': [],
