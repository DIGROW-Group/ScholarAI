        ...
        try:
            ...
            with TemporaryDirectory() as temp_dir:
                for i, document in enumerate(documents):
                    ...
                    # Extract KPIs from this individual document
            ...
            current_data['processing_stage'] = 'kpis_extracted'
            if output_md_path:
            ...
            log_progress(f"🔍 DEBUG: About to save profile with stage: {current_data['processing_stage']}")
            log_progress(f"🔍 DEBUG: KPIs count: {len(current_data.get('extracted_kpis', {}))}")
            log_progress(f"🔍 DEBUG: Ratios count: {len(current_data.get('computed_ratios', {}))}")
            ...
            log_progress(f"🔍 DEBUG: Final verification - stage: {verification_data.get('processing_stage', 'NOT_SET')}")
            log_progress(f"🔍 DEBUG: Final verification - KPIs count: {len(verification_data.get('extracted_kpis', {}))}")
            log_progress(f"🔍 DEBUG: Final verification - Ratios count: {len(verification_data.get('computed_ratios', {}))}")
            ...
            try:
                ...
                # Debug: Log the state before any changes
                log_progress(f"🔍 DEBUG: Finally block - current stage: {current_data.get('processing_stage', 'NOT_SET')}")
                
                # Only set to failed if processing actually failed (not if it's in a valid intermediate state)
                if 'processing_stage' not in current_data:
                    log_progress("🔍 DEBUG: No processing_stage found, setting to failed")
                    current_data['processing_stage'] = 'failed'
                elif current_data['processing_stage'] not in ['completed', 'kpis_extracted', 'kpis_confirmed', 'generating_report']:
                    # Only set to failed if it's not in a valid processing state
                    if current_data['processing_stage'] not in ['initializing', 'waiting_for_documents', 'processing_documents', 'saving_markdown', 'extracting_kpis']:
                        log_progress(f"🔍 DEBUG: Stage {current_data['processing_stage']} not in valid states, setting to failed")
                        current_data['processing_stage'] = 'failed'
                    else:
                        log_progress(f"🔍 DEBUG: Stage {current_data['processing_stage']} is valid, keeping it")
                else:
                    log_progress(f"🔍 DEBUG: Stage {current_data['processing_stage']} is in final states, keeping it")
                
                profile.profile_data = current_data
                db.session.commit()
                
                # Debug: Log final state after finally block
                log_progress(f"🔍 DEBUG: Finally block - final stage: {current_data.get('processing_stage', 'NOT_SET')}")
...
def _compute_financial_ratios(kpis_data: dict) -> dict:
    ...
    # Always recalculate to ensure we use the latest corrected values
    for year in processed_years:
        tres_actif = base_kpis.get(f'tresorerie_actif_{year}', 0) or 0
        tres_passif = base_kpis.get(f'tresorerie_passif_{year}', 0) or 0
